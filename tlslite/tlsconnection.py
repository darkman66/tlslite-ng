...
class TLSConnection(TLSRecordLayer):
    ...
    def keyingMaterialExporter(self, label, length=20):
        ...
        """
        if label in (b"server finished", b"client finished", b"master secret", b"key expansion"):
            raise ValueError("Forbidden label value")
        if self.version < (3, 1):
            raise ValueError("Supported only in TLSv1.0 and later")
        elif self.version < (3, 3):
            return PRF(self.session.masterSecret, label, self._clientRandom + self._serverRandom, length)
        elif self.version == (3, 3):
            if self.session.cipherSuite in CipherSuite.sha384PrfSuites:
                return PRF_1_2_SHA384(self.session.masterSecret, label, self._clientRandom + self._serverRandom, length)
            else:
                return PRF_1_2(self.session.masterSecret, label, self._clientRandom + self._serverRandom, length)
        elif self.version == (3, 4):
            prf = "sha256"
            if self.session.cipherSuite in CipherSuite.sha384PrfSuites:
                prf = "sha384"
            secret = derive_secret(self.session.exporterMasterSecret, label, None, prf)
    ...
    @deprecated_params({"async_": "async"}, "'{old_name}' is a keyword in Python 3.7, use" "'{new_name}'")
    def handshakeClientAnonymous(self, session=None, settings=None, checker=None, serverName=None, async_=False):
        ...
        """
        handshaker = self._handshakeClientAsync(
            anonParams=(True), session=session, settings=settings, checker=checker, serverName=serverName
        )
        if async_:
            return handshaker
    ...
    def handshakeClientSRP(
        self, username, password, session=None, settings=None, checker=None, reqTack=True, serverName=None, async_=False
    ):
    ...
    def handshakeClientCert(
        ...
        """
        handshaker = self._handshakeClientAsync(
            certParams=(certChain, privateKey),
            session=session,
            settings=settings,
            checker=checker,
            serverName=serverName,
    ...
    def _handshakeClientAsync(
        ...
        handshaker = self._handshakeClientAsyncHelper(
            srpParams=srpParams,
    ...
    def _handshakeClientAsyncHelper(
        self, srpParams, certParams, anonParams, session, settings, serverName, nextProtos, reqTack, alpn
    ):
        ...
        if serverName and not is_valid_hostname(serverName):
            raise ValueError("Caller provided invalid server host name: {0}".format(serverName))
        ...
        if (settings.maxVersion > (3, 3) and self.version <= (3, 3)) and (
            serverHello.random[-8:] == TLS_1_2_DOWNGRADE_SENTINEL
            or serverHello.random[-8:] == TLS_1_1_DOWNGRADE_SENTINEL
        ):
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Connection terminated because " "of downgrade protection."
            ):
                yield result
        ...
        if (
            ...
            and self.version < (3, 3)
            and serverHello.random[-8:] == TLS_1_1_DOWNGRADE_SENTINEL
        ):
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Connection terminated because " "of downgrade protection."
            ):
        ...
        if ext and ext.version > (3, 3):
            for result in self._clientTLS13Handshake(
                settings, session, clientHello, clientCertChain, privateKey, serverHello
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        for result in self._clientResume(
            session, serverHello, clientHello.random, settings.cipherImplementations, nextProto, settings
        ):
            if result in (0, 1):
                yield result
            else:
        ...
        # finishes reading the post-ServerHello messages.
            ...
            keyExchange = DHE_RSAKeyExchange(cipherSuite, clientHello, serverHello, None)

        elif cipherSuite in CipherSuite.ecdhAllSuites:
            acceptedCurves = self._curveNamesToList(settings)
            keyExchange = ECDHE_RSAKeyExchange(cipherSuite, clientHello, serverHello, None, acceptedCurves)
        ...
        self.session.create(
            ...
            serverCertChain,
            tackExt,
            (serverHello.tackExt is not None),
            serverName,
    ...
    def _clientSendClientHello(
        self, settings, session, srpUsername, srpParams, certParams, anonParams, serverName, nextProtos, reqTack, alpn
    ):
        ...
        if settings.useEncryptThenMAC:
            extensions.append(TLSExtension().create(ExtensionType.encrypt_then_mac, bytearray(0)))
        ...
        if next((i for i in settings.versions if i > (3, 3)), None):
            ...
            if certParams and certParams[1]:
                extensions.append(TLSExtension(extType=ExtensionType.post_handshake_auth).create(bytearray(b"")))
                self._client_keypair = certParams

            # fake session_id for middlebox compatibility mode
            session_id = getRandomBytes(32)

            extensions.append(SupportedVersionsExtension().create(settings.versions))
            ...
            ext = PskKeyExchangeModesExtension().create([getattr(PskKeyExchangeMode, i) for i in settings.psk_modes])
            extensions.append(ext)

        groups = []
        # Send the ECC extensions only if we advertise ECC ciphers
        if next((cipher for cipher in cipherSuites if cipher in CipherSuite.ecdhAllSuites), None) is not None:
        ...
        if next((cipher for cipher in cipherSuites if cipher in CipherSuite.dhAllSuites), None) is not None:
        ...
        if settings.use_heartbeat_extension:
            extensions.append(HeartbeatExtension().create(HeartbeatMode.PEER_ALLOWED_TO_SEND))
            self.heartbeat_can_receive = True

        if settings.record_size_limit:
            extensions.append(RecordSizeLimitExtension().create(settings.record_size_limit))
        ...
        if (settings.pskConfigs or (session and session.tickets)) and settings.maxVersion >= (3, 4):
            ...
            if session and session.tickets:
                ...
                # 7 days
                session.tickets[:] = (
                    i for i in session.tickets if i.time + i.ticket_lifetime > now and i.time + 7 * 24 * 60 * 60 > now
                )
                if session.tickets:
                    ...
                    # is in ms
                    ticket_time = int(time.time() * 1000 - ticket.time * 1000 + ticket.ticket_age_add) % 2**32
                    idens.append(PskIdentity().create(ticket.ticket, ticket_time))
                    binder_len = 48 if session.cipherSuite in CipherSuite.sha384PrfSuites else 32
            ...
            if idens:
                ...
                HandshakeHelpers.update_binders(
                    ...
                    settings.pskConfigs,
                    session.tickets if session else None,
                    session.resumptionMasterSecret if session else None,
                )
        print("clientHello", clientHello)
        print("clientHello", dir(clientHello))
        yield self.send_hello_data(clientHello)
        yield clientHello

    def send_hello_data(self, client_hello):
    ...
    def _clientGetServerHello(self, settings, session, clientHello):
        ...
        if hasattr(self, "get_server_hello"):
            result = self.get_server_hello()
        else:
        ...
        if result.random == TLS_1_3_HRR and ext and ext.version > (3, 3):
            ...
            bad_ext = next((i for i in hello_retry.extensions if i.extType not in ch_ext_types), None)
            if bad_ext:
                bad_ext = ExtensionType.toStr(bad_ext)
                for result in self._sendError(
                    AlertDescription.unsupported_extension, ("Unexpected extension in HRR: " "{0}").format(bad_ext)
                ):
                    yield result
            ...
            sr_key_share_ext = hello_retry.getExtension(ExtensionType.key_share)
            if sr_key_share_ext:
                group_id = sr_key_share_ext.selected_group
                # check if group selected by server is valid
                groups_ext = clientHello.getExtension(ExtensionType.supported_groups)
                if group_id not in groups_ext.groups:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "Server selected group we " "did not advertise"
                    ):
                        yield result

                cl_key_share_ext = clientHello.getExtension(ExtensionType.key_share)
                # check if the server didn't ask for a group we already sent
                if next((entry for entry in cl_key_share_ext.client_shares if entry.group == group_id), None):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "Server selected group we " "did sent the key share " "for"
                    ):
                        yield result
            ...
            if not cookie and not sr_key_share_ext:
                ...
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Received HRR did not cause " "update to Client Hello"
                ):
                    yield result
            ...
            if clientHello.session_id != hello_retry.session_id:
                for result in self._sendError(
                    ...
                    "Received HRR session_id does not match the one in " "ClientHello",
                ):
                    yield result
        ...
        # Check ServerHello
        if hello_retry and hello_retry.cipher_suite != serverHello.cipher_suite:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "server selected different cipher " "in HRR and Server Hello"
            ):
                yield result
        if real_version < settings.minVersion:
            for result in self._sendError(
                AlertDescription.protocol_version,
                "Too old version: {0} (min: {1})".format(real_version, settings.minVersion),
            ):
                yield result
        if real_version > settings.maxVersion and real_version not in settings.versions:
            for result in self._sendError(
                AlertDescription.protocol_version,
                "Too new version: {0} (max: {1})".format(real_version, settings.maxVersion),
            ):
                yield result
        if real_version > (3, 3) and serverHello.session_id != clientHello.session_id:
            for result in self._sendError(
                AlertDescription.illegal_parameter,
                "Received ServerHello session_id does not match the one " "in ClientHello",
            ):
                yield result
        print(type(clientHello))
        cipherSuites = CipherSuite.filterForVersion(
            clientHello.cipher_suites, minVersion=real_version, maxVersion=real_version
        )
        print("server cipherSuites", serverHello.cipher_suite)
        print("client cipherSuites", cipherSuites)
        if serverHello.cipher_suite not in cipherSuites:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Server responded with incorrect ciphersuite"
            ):
                yield result
        if serverHello.certificate_type not in clientHello.certificate_types:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Server responded with incorrect certificate type"
            ):
                yield result
        if serverHello.compression_method != 0:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Server responded with incorrect compression method"
            ):
                yield result
        if serverHello.tackExt:
            if not clientHello.tack:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Server responded with unrequested Tack Extension"
                ):
                    yield result
            if not serverHello.tackExt.verifySignatures():
                for result in self._sendError(
                    AlertDescription.decrypt_error, "TackExtension contains an invalid signature"
                ):
                    yield result
        if serverHello.next_protos and not clientHello.supports_npn:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Server responded with unrequested NPN Extension"
            ):
                yield result
        if not serverHello.getExtension(ExtensionType.extended_master_secret) and settings.requireExtendedMasterSecret:
            for result in self._sendError(
                AlertDescription.insufficient_security, "Negotiation of Extended master Secret failed"
            ):
                yield result
        alpnExt = serverHello.getExtension(ExtensionType.alpn)
        if alpnExt:
            if not alpnExt.protocol_names or len(alpnExt.protocol_names) != 1:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Server responded with invalid ALPN extension"
                ):
                    yield result
            clntAlpnExt = clientHello.getExtension(ExtensionType.alpn)
            if not clntAlpnExt:
                for result in self._sendError(
                    AlertDescription.unsupported_extension, "Server sent ALPN extension without one in " "client hello"
                ):
                    yield result
            if alpnExt.protocol_names[0] not in clntAlpnExt.protocol_names:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Server selected ALPN protocol we did not advertise"
                ):
                    yield result
        ...
        if heartbeat_ext:
            if not settings.use_heartbeat_extension:
                for result in self._sendError(
                    ...
                    "Server sent Heartbeat extension without one in " "client hello",
                ):
                    yield result
            if heartbeat_ext.mode == HeartbeatMode.PEER_ALLOWED_TO_SEND and settings.heartbeat_response_callback:
                self.heartbeat_can_send = True
                self.heartbeat_response_callback = settings.heartbeat_response_callback
            elif (
                heartbeat_ext.mode == HeartbeatMode.PEER_NOT_ALLOWED_TO_SEND or not settings.heartbeat_response_callback
            ):
                self.heartbeat_can_send = False
            else:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Server responded with invalid Heartbeat extension"
                ):
                    yield result
            self.heartbeat_supported = True
        size_limit_ext = serverHello.getExtension(ExtensionType.record_size_limit)
        if size_limit_ext:
            if size_limit_ext.record_size_limit is None:
                for result in self._sendError(AlertDescription.decode_error, "Malformed record_size_limit extension"):
                    yield result
            ...
            if not 64 <= size_limit_ext.record_size_limit <= 2**14:
                for result in self._sendError(
                    AlertDescription.illegal_parameter,
                    "Server responed with invalid value in " "record_size_limit extension",
                ):
                    yield result
    ...
    def _clientTLS13Handshake(self, settings, session, clientHello, clientCertChain, privateKey, serverHello):
        ...
        if sr_kex:
            ...
            cl_kex = next((i for i in cl_key_share_ex.client_shares if i.group == sr_kex.group), None)
            if cl_kex is None:
        ...
        if sr_psk:
            ...
            if psk:
                ...
                resuming = True
                psk = HandshakeHelpers.calc_res_binder_psk(ident, session.resumptionMasterSecret, session.tickets)
        else:
        ...
        sr_handshake_traffic_secret = derive_secret(secret, bytearray(b"s hs traffic"), self._handshake_hash, prfName)
        cl_handshake_traffic_secret = derive_secret(secret, bytearray(b"c hs traffic"), self._handshake_hash, prfName)
        ...
        size_limit_ext = encrypted_extensions.getExtension(ExtensionType.record_size_limit)
        if size_limit_ext and not settings.record_size_limit:
            for result in self._sendError(
                AlertDescription.illegal_parameter,
                "Server sent record_size_limit extension despite us not " "advertising it",
            ):
                yield result
        if size_limit_ext:
            if size_limit_ext.record_size_limit is None:
            ...
            if not 64 <= size_limit_ext.record_size_limit <= 2**14 + 1:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Invalid valid in record_size_limit extension"
                ):
                    yield result
        ...
        if not sr_psk:
            ...
            for result in self._getMsg(ContentType.handshake, HandshakeType.certificate_verify):
                if result in (0, 1):
                    yield result
                else:
                    break
            ...
            signature_context = KeyExchange.calcVerifyBytes(
                (3, 4), srv_cert_verify_hh, signature_scheme, None, None, None, prfName, b"server"
            )
            ...
            if signature_scheme in (SignatureScheme.ed25519, SignatureScheme.ed448):
                ...
                matching_hash = self._curve_name_to_hash_name(publicKey.curve_name)
                if hash_name != matching_hash:
                    raise TLSIllegalParameterException(
                        "server selected signature method invalid for the " "certificate it presented (curve mismatch)"
                    )
            ...
            if not method(certificate_verify.signature, signature_context, pad_type, hash_name, salt_len):
                raise TLSDecryptionFailed("server Certificate Verify " "signature " "verification failed")

        transcript_hash = self._handshake_hash.digest(prfName)

        for result in self._getMsg(ContentType.handshake, HandshakeType.finished, prf_size):
            if result in (0, 1):
                yield result
            else:
                break
        ...
        finished_key = HKDF_expand_label(sr_handshake_traffic_secret, b"finished", b"", prf_size, prfName)
        ...
        cl_app_traffic = derive_secret(secret, bytearray(b"c ap traffic"), server_finish_hs, prfName)
        sr_app_traffic = derive_secret(secret, bytearray(b"s ap traffic"), server_finish_hs, prfName)

        if certificate_request:
            client_certificate = Certificate(serverHello.certificate_type, self.version)
            if clientCertChain:
                # Check to make sure we have the same type of certificates the
                # server requested
                if serverHello.certificate_type == CertificateType.x509 and not isinstance(
                    clientCertChain, X509CertChain
                ):
                    for result in self._sendError(
                        AlertDescription.handshake_failure, "Client certificate is of wrong type"
                    ):
                        yield result
            ...
            if clientCertChain and privateKey:
                ...
                if not valid_sig_algs:
                    for result in self._sendError(AlertDescription.missing_extension, "No Signature Algorithms found"):
                        yield result

                availSigAlgs = self._sigHashesToList(settings, privateKey, clientCertChain, version=(3, 4))
                signature_scheme = getFirstMatching(availSigAlgs, valid_sig_algs)
                scheme = SignatureScheme.toRepr(signature_scheme)
                signature_scheme = getattr(SignatureScheme, scheme)

                signature_context = KeyExchange.calcVerifyBytes(
                    (3, 4), self._handshake_hash, signature_scheme, None, None, None, prfName, b"client"
                )
                ...
                signature = sig_func(signature_context, pad_type, hash_name, salt_len)
                if not ver_func(signature, signature_context, pad_type, hash_name, salt_len):
                    for result in self._sendError(
                        AlertDescription.internal_error, "Certificate Verify signature failed"
                    ):
                        yield result
        ...
        exporter_master_secret = derive_secret(secret, bytearray(b"exp master"), self._handshake_hash, prfName)

        self._recordLayer.calcTLS1_3PendingState(
            serverHello.cipher_suite, cl_app_traffic, sr_app_traffic, settings.cipherImplementations
        )
        ...
        cl_finished_key = HKDF_expand_label(cl_handshake_traffic_secret, b"finished", b"", prf_size, prfName)
        cl_verify_data = secureHMAC(cl_finished_key, self._handshake_hash.digest(prfName), prfName)
        ...
        resumption_master_secret = derive_secret(secret, bytearray(b"res master"), self._handshake_hash, prfName)
    ...
    def _clientResume(self, session, serverHello, clientRandom, cipherImplementations, nextProto, settings):
        ...
        if session and session.sessionID and serverHello.session_id == session.sessionID:
            ...
            if serverHello.cipher_suite != session.cipherSuite:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Server's ciphersuite doesn't match session"
                ):
                    yield result

            # Calculate pending connection states
            self._calcPendingStates(
                session.cipherSuite, session.masterSecret, clientRandom, serverHello.random, cipherImplementations
            )

            # Exchange ChangeCipherSpec and Finished messages
            for result in self._getFinished(session.masterSecret, session.cipherSuite):
                yield result
            # buffer writes so that CCS and Finished go out in one TCP packet
            self.sock.buffer_writes = True
            for result in self._sendFinished(session.masterSecret, session.cipherSuite, nextProto, settings=settings):
                yield result
            self.sock.flush()
    ...
    def _clientKeyExchange(
        ...
        if hasattr(self, "get_server_certificate"):
            ...
            if (
                cipherSuite in CipherSuite.certAllSuites
                or cipherSuite in CipherSuite.ecdheEcdsaSuites
                or cipherSuite in CipherSuite.dheDsaSuites
            ):
                for result in self._getMsg(ContentType.handshake, HandshakeType.certificate, certificateType):
                    if result in (0, 1):
                        yield result
                    else:
                        break
        ...
        if cipherSuite not in CipherSuite.certSuites:
            if hasattr(self, "get_server_key_exchange"):
                ...
                for result in self._getMsg(ContentType.handshake, HandshakeType.server_key_exchange, cipherSuite):
                    if result in (0, 1):
                        yield result
                    else:
                        break
        ...
        if hasattr(self, "get_certificate_request"):
            result = self.get_certificate_request()
        else:
            for result in self._getMsg(
                ContentType.handshake, (HandshakeType.certificate_request, HandshakeType.server_hello_done)
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        if isinstance(result, CertificateRequest):
            ...
            if (
                cipherSuite not in CipherSuite.certAllSuites
                ...
                for result in self._sendError(
                    AlertDescription.unexpected_message, "Certificate Request with incompatible cipher suite"
                ):
                    yield result

            # we got CertificateRequest so now we'll get ServerHelloDone
            for result in self._getMsg(ContentType.handshake, HandshakeType.server_hello_done):
                if result in (0, 1):
                    yield result
                else:
                    break
        ...
        if (
            ...
            for result in self._clientGetKeyFromChain(serverCertificate, settings, tackExt):
                if result in (0, 1):
                    yield result
                else:
                    break
            ...
            if serverKeyExchange:
                valid_sig_algs = self._sigHashesToList(settings, certList=serverCertChain)
                # try:
                ...
                KeyExchange.verifyServerKeyExchange(
                    serverKeyExchange, publicKey, clientRandom, serverRandom, valid_sig_algs
                )
        ...
        if serverKeyExchange:
            ...
            if (
                self.version >= (3, 3)
                and (cipherSuite in CipherSuite.certAllSuites or cipherSuite in CipherSuite.ecdheEcdsaSuites)
                and cipherSuite not in CipherSuite.certSuites
            ):
                self.serverSigAlg = (serverKeyExchange.hashAlg, serverKeyExchange.signAlg)
        ...
        if certificateRequest:
            ...
            if self.version == (3, 3) and not [
                sig for sig in certificateRequest.supported_signature_algs if sig[1] == SignatureAlgorithm.rsa
            ]:
                for result in self._sendError(
                    AlertDescription.handshake_failure,
                    "Server doesn't accept any sigalgs we support: " + str(certificateRequest.supported_signature_algs),
                ):
                    yield result
            ...
            if clientCertChain:
                ...
                if certificateType == CertificateType.x509 and not isinstance(clientCertChain, X509CertChain):
                    for result in self._sendError(
                        AlertDescription.handshake_failure, "Client certificate is of wrong type"
                    ):
                        yield result
        ...
        try:
            ...
            premasterSecret = keyExchange.processServerKeyExchange(publicKey, ske)
        except TLSInsufficientSecurity as e:
            for result in self._sendError(AlertDescription.insufficient_security, e):
                yield result
        except TLSIllegalParameterException as e:
            for result in self._sendError(AlertDescription.illegal_parameter, e):
                yield result
        ...
        if certificateRequest and privateKey:
            valid_sig_algs = self._sigHashesToList(settings, privateKey, clientCertChain)
            try:
                ...
                for result in self._sendError(AlertDescription.internal_error, exception):
                    yield result
    ...
    def _check_certchain_with_settings(self, cert_chain, settings):
        ...
        if cert_type == "ecdsa":
            ...
            if self.version <= (3, 3) and curve_name not in settings.eccCurves:
                for result in self._sendError(
                    AlertDescription.handshake_failure,
                    "Peer sent certificate with curve we did not " "advertise support for: {0}".format(curve_name),
                ):
                    yield result
            if self.version >= (3, 4):
                if curve_name not in ("secp256r1", "secp384r1", "secp521r1"):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Peer sent certificate with curve not supported " "in TLS 1.3: {0}".format(curve_name),
                    ):
                        yield result
                ...
                if sig_alg_for_curve not in settings.ecdsaSigHashes:
                    for result in self._sendError(
                        ...
                        "did not advertise support for: {0}".format(curve_name),
                    ):
                        yield result
            ...
            if self.version < (3, 3):
                for result in self._sendError(
                    AlertDescription.illegal_parameter,
                    "Peer sent certificate incompatible with negotiated " "TLS version",
                ):
                    yield result
            if cert_type not in settings.more_sig_schemes:
                for result in self._sendError(
                    AlertDescription.handshake_failure,
                    "Peer sent certificate we did not advertise support " "for: {0}".format(cert_type),
                ):
                    yield result
            ...
            if len(publicKey) < settings.minKeySize:
                for result in self._sendError(
                    AlertDescription.handshake_failure, "Other party's public key too small: %d" % len(publicKey)
                ):
                    yield result
            if len(publicKey) > settings.maxKeySize:
                for result in self._sendError(
                    AlertDescription.handshake_failure, "Other party's public key too large: %d" % len(publicKey)
                ):
                    yield result
        yield publicKey
    ...
    def _clientGetKeyFromChain(self, certificate, settings, tack_ext=None):
        ...
        if not cert_chain or cert_chain.getNumCerts() == 0:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Other party sent a Certificate message without " "certificates"
            ):
                yield result

        for result in self._check_certchain_with_settings(cert_chain, settings):
            if result in (0, 1):
                yield result
            else:
                break
            ...
            if tack_ext and tack_ext.tacks:
                for tack in tack_ext.tacks:
                    if not cert_chain.checkTack(tack):
                        for result in self._sendError(
                            AlertDescription.illegal_parameter, "Other party's TACK doesn't match their public key"
                        ):
                            yield result
    ...
    def handshakeServer(
        ...
        :raises tlslite.errors.TLSAuthenticationError: If the checker
            doesn't like the other party's authentication credentials.
        """
        for result in self.handshakeServerAsync(
            verifierDB,
            certChain,
            privateKey,
            reqCert,
    ...
    def _handshakeServerAsyncHelper(
        self,
        verifierDB,
        cert_chain,
        privateKey,
        ...
        if (
            ...
            and (not cert_chain)
            and not anon
            and not settings.pskConfigs
            and not settings.virtual_hosts
        ):
        ...
        for result in self._serverGetClientHello(
            settings, privateKey, cert_chain, verifierDB, sessionCache, anon, alpn, sni
        ):
            if result in (0, 1):
                yield result
            elif result == None:
                ...
                break
        (clientHello, version, cipherSuite, sig_scheme, privateKey, cert_chain) = result

        # in TLS 1.3 the handshake is completely different
        # (extensions go into different messages, format of messages is
        # different, etc.)
        if version > (3, 3):
            for result in self._serverTLS13Handshake(
                settings, clientHello, cipherSuite, privateKey, cert_chain, version, sig_scheme, alpn, reqCert
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        if not cipherSuite in CipherSuite.certAllSuites and not cipherSuite in CipherSuite.ecdheEcdsaSuites:
            tacks = None
        ...
        if (
            settings.useEncryptThenMAC
            and clientHello.getExtension(ExtensionType.encrypt_then_mac)
            and cipherSuite not in CipherSuite.streamSuites
            and cipherSuite not in CipherSuite.aeadSuites
        ):
            extensions.append(TLSExtension().create(ExtensionType.encrypt_then_mac, bytearray(0)))
            self._recordLayer.encryptThenMAC = True
        ...
        if settings.useExtendedMasterSecret:
            if clientHello.getExtension(ExtensionType.extended_master_secret):
                ...
                for result in self._sendError(
                    AlertDescription.insufficient_security, "Failed to negotiate Extended Master Secret"
                ):
                    yield result
        ...
        if alpnExt and alpn:
            for protoName in alpnExt.protocol_names:
                ...
                for result in self._sendError(
                    AlertDescription.no_application_protocol, "No mutually supported application layer protocols"
                ):
                    yield result
        ...
        if renegoExt:
            if renegoExt.renegotiated_connection:
                for result in self._sendError(
                    AlertDescription.handshake_failure,
                    "Non empty renegotiation info extension in " "initial Client Hello",
                ):
                    yield result
            secureRenego = True
        elif CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV in clientHello.cipher_suites:
            secureRenego = True
        if secureRenego:
        ...
        if clientHello.getExtension(ExtensionType.record_size_limit) and settings.record_size_limit:
            # in TLS 1.2 and earlier we can select at most 2^14B records
            extensions.append(RecordSizeLimitExtension().create(min(2**14, settings.record_size_limit)))
        ...
        if cipherSuite in CipherSuite.srpAllSuites:
            for result in self._serverSRPKeyExchange(
                clientHello, serverHello, verifierDB, cipherSuite, privateKey, cert_chain, settings
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        # Perform a certificate-based key exchange
            ...
            cipherSuite in CipherSuite.certSuites
            or cipherSuite in CipherSuite.dheCertSuites
            or cipherSuite in CipherSuite.dheDsaSuites
            or cipherSuite in CipherSuite.ecdheCertSuites
            or cipherSuite in CipherSuite.ecdheEcdsaSuites
        ):
            try:
                sig_hash_alg, cert_chain, privateKey = self._pickServerKeyExchangeSig(
                    settings, clientHello, cert_chain, privateKey
                )
            ...
            if cipherSuite in CipherSuite.certSuites:
                keyExchange = RSAKeyExchange(cipherSuite, clientHello, serverHello, privateKey)
            elif cipherSuite in CipherSuite.dheCertSuites or cipherSuite in CipherSuite.dheDsaSuites:
                dhGroups = self._groupNamesToList(settings)
                keyExchange = DHE_RSAKeyExchange(
                    cipherSuite, clientHello, serverHello, privateKey, settings.dhParams, dhGroups
                )
            elif cipherSuite in CipherSuite.ecdheCertSuites or cipherSuite in CipherSuite.ecdheEcdsaSuites:
                acceptedCurves = self._curveNamesToList(settings)
                defaultCurve = getattr(GroupName, settings.defaultCurve)
                keyExchange = ECDHE_RSAKeyExchange(
                    cipherSuite, clientHello, serverHello, privateKey, acceptedCurves, defaultCurve
                )
            else:
                assert False
            for result in self._serverCertKeyExchange(
                clientHello, serverHello, sig_hash_alg, cert_chain, keyExchange, reqCert, reqCAs, cipherSuite, settings
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        elif cipherSuite in CipherSuite.anonSuites or cipherSuite in CipherSuite.ecdhAnonSuites:
            if cipherSuite in CipherSuite.anonSuites:
                dhGroups = self._groupNamesToList(settings)
                keyExchange = ADHKeyExchange(cipherSuite, clientHello, serverHello, settings.dhParams, dhGroups)
            else:
                acceptedCurves = self._curveNamesToList(settings)
                defaultCurve = getattr(GroupName, settings.defaultCurve)
                keyExchange = AECDHKeyExchange(cipherSuite, clientHello, serverHello, acceptedCurves, defaultCurve)
            for result in self._serverAnonKeyExchange(serverHello, keyExchange, cipherSuite):
                if result in (0, 1):
                    yield result
                else:
                    break
        ...
        for result in self._serverFinished(
            premasterSecret,
            clientHello.random,
            serverHello.random,
            cipherSuite,
        ...
        self.session = Session()
        if cipherSuite in CipherSuite.certAllSuites or cipherSuite in CipherSuite.ecdheEcdsaSuites:
    ...
    def request_post_handshake_auth(self, settings=None):
        ...
        if not valid_sig_algs:
            raise ValueError("No signature algorithms enabled in " "HandshakeSettings")
    ...
    def _derive_key_iv(nonce, user_key, settings):
        ...
        ticket_secret = derive_secret(secret, bytearray(b"SessionTicket secret"), None, prf_name)

        key = HKDF_expand_label(ticket_secret, b"key", b"", len(user_key), prf_name)
    ...
    def _serverSendTickets(self, settings):
        ...
        for _ in range(settings.ticket_count):
            ...
            ticket.create(
                self.session.resumptionMasterSecret,
                self.version,
            ...
            key, iv = self._derive_key_iv(nonce, settings.ticketKeys[0], settings)
            ...
            new_ticket.create(
                settings.ticketLifetime, getRandomNumber(1, 8**4), ticket.nonce, nonce + encrypted_ticket, []
            )
    ...
    def _tryDecrypt(self, settings, identity):
        ...
        for user_key in settings.ticketKeys:
            ...
            prf = "sha384" if ticket.cipher_suite in CipherSuite.sha384PrfSuites else "sha256"

            new_sess_ticket = NewSessionTicket()
            new_sess_ticket.ticket_nonce = ticket.nonce
            new_sess_ticket.ticket = identity.identity

            psk = HandshakeHelpers.calc_res_binder_psk(identity, ticket.master_secret, [new_sess_ticket])
    ...
    def _serverTLS13Handshake(
        self, settings, clientHello, cipherSuite, privateKey, serverCertChain, version, scheme, srv_alpns, reqCert
    ):
        ...
        if share:
            ...
            for group_name in chain(settings.keyShares, settings.eccCurves, settings.dhGroups):
                ...
                for result in self._sendError(AlertDescription.internal_error, "HRR did not work?!"):
                    yield result
        ...
        if (
            psks
            and (PskKeyExchangeMode.psk_dhe_ke in psk_types.modes or PskKeyExchangeMode.psk_ke in psk_types.modes)
            and (settings.pskConfigs or settings.ticketKeys)
        ):
            for i, ident in enumerate(psks.identities):
                ticket = None
                external = True
                match = [j for j in settings.pskConfigs if j[0] == ident.identity]
                if not match:
                ...
                try:
                    HandshakeHelpers.verify_binder(
                        clientHello, self._pre_client_hello_handshake_hash, selected_psk, psk, psk_hash, external
                    )
                except TLSIllegalParameterException as e:
                    for result in self._sendError(AlertDescription.illegal_parameter, str(e)):
                        yield result
                break
        ...
        if (psk and PskKeyExchangeMode.psk_dhe_ke in psk_types.modes and "psk_dhe_ke" in settings.psk_modes) or (
            ...
            try:
                shared_sec = kex.calc_shared_key(key_share.private, cl_key_share.key_exchange)
            except TLSIllegalParameterException as alert:
                for result in self._sendError(AlertDescription.illegal_parameter, str(alert)):
                    yield result

            sh_extensions.append(ServerKeyShareExtension().create(key_share))
        elif psk is not None and PskKeyExchangeMode.psk_ke in psk_types.modes and "psk_ke" in settings.psk_modes:
            shared_sec = bytearray(prf_size)
        else:
            for result in self._sendError(
                AlertDescription.handshake_failure, "Could not find acceptable PSK identity nor certificate"
            ):
                yield result
        ...
        if selected_psk is not None:
            sh_extensions.append(SrvPreSharedKeyExtension().create(selected_psk))

        serverHello = ServerHello()
        # in TLS1.3 the version selected is sent in extension, (3, 3) is
        # just dummy value to workaround broken middleboxes
        serverHello.create((3, 3), getRandomBytes(32), clientHello.session_id, cipherSuite, extensions=sh_extensions)
        ...
        sr_handshake_traffic_secret = derive_secret(secret, bytearray(b"s hs traffic"), self._handshake_hash, prf_name)
        cl_handshake_traffic_secret = derive_secret(secret, bytearray(b"c hs traffic"), self._handshake_hash, prf_name)
        self.version = version
        self._recordLayer.calcTLS1_3PendingState(
            cipherSuite, cl_handshake_traffic_secret, sr_handshake_traffic_secret, settings.cipherImplementations
        )
        ...
        if clientHello.getExtension(ExtensionType.cookie):
            ...
            groups += [getattr(GroupName, i) for i in settings.eccCurves if getattr(GroupName, i) not in groups]
            groups += [getattr(GroupName, i) for i in settings.dhGroups if getattr(GroupName, i) not in groups]
            if groups:
                ext.create(groups)
        ...
        if clientHello.getExtension(ExtensionType.heartbeat):
            if settings.use_heartbeat_extension:
                ee_extensions.append(HeartbeatExtension().create(HeartbeatMode.PEER_ALLOWED_TO_SEND))
        ...
        if selected_psk is None:
            ...
            signature_context = KeyExchange.calcVerifyBytes(
                (3, 4), self._handshake_hash, signature_scheme, None, None, None, prf_name, b"server"
            )
            ...
            signature = sig_func(signature_context, padType, hashName, saltLen)
            if not ver_func(signature, signature_context, padType, hashName, saltLen):
                for result in self._sendError(AlertDescription.internal_error, "Certificate Verify signature failed"):
                    yield result
            certificate_verify.create(signature, signature_scheme)

            self._queue_message(certificate_verify)

        finished_key = HKDF_expand_label(sr_handshake_traffic_secret, b"finished", b"", prf_size, prf_name)
        verify_data = secureHMAC(finished_key, self._handshake_hash.digest(prf_name), prf_name)
        ...
        cl_app_traffic = derive_secret(secret, bytearray(b"c ap traffic"), self._handshake_hash, prf_name)
        sr_app_traffic = derive_secret(secret, bytearray(b"s ap traffic"), self._handshake_hash, prf_name)
        self._recordLayer.calcTLS1_3PendingState(
            serverHello.cipher_suite, cl_app_traffic, sr_app_traffic, settings.cipherImplementations
        )
        ...
        if client_cert_chain and client_cert_chain.getNumCerts():
            ...
            valid_sig_algs = self._sigHashesToList(settings, certList=client_cert_chain, version=(3, 4))
            if signature_scheme not in valid_sig_algs:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Invalid signature on Certificate Verify"
                ):
                    yield result

            signature_context = KeyExchange.calcVerifyBytes(
                (3, 4), cli_cert_verify_hh, signature_scheme, None, None, None, prf_name, b"client"
            )
            ...
            if not ver_func(certificate_verify.signature, signature_context, pad_type, hash_name, salt_len):
                for result in self._sendError(AlertDescription.decrypt_error, "signature verification failed"):
                    yield result

        # as both exporter and resumption master secrets include handshake
        # transcript, we need to derive them early
        exporter_master_secret = derive_secret(secret, bytearray(b"exp master"), self._handshake_hash, prf_name)

        # verify Finished of client
        cl_finished_key = HKDF_expand_label(cl_handshake_traffic_secret, b"finished", b"", prf_size, prf_name)
        cl_verify_data = secureHMAC(cl_finished_key, self._handshake_hash.digest(prf_name), prf_name)
        for result in self._getMsg(ContentType.handshake, HandshakeType.finished, prf_size):
            if result in (0, 1):
                yield result
            else:
        ...
        if cl_finished.verify_data != cl_verify_data:
            for result in self._sendError(AlertDescription.decrypt_error, "Finished value is not valid"):
                yield result

        # disallow CCS messages after handshake
        self._middlebox_compat_mode = False

        resumption_master_secret = derive_secret(secret, bytearray(b"res master"), self._handshake_hash, prf_name)
    ...
    def _serverGetClientHello(self, settings, private_key, cert_chain, verifierDB, sessionCache, anon, alpn, sni):
        ...
        # Get ClientHello
        for result in self._getMsg(ContentType.handshake, HandshakeType.client_hello):
            if result in (0, 1):
                yield result
            else:
                break
        ...
        if real_version < settings.minVersion:
            ...
            for result in self._sendError(
                AlertDescription.protocol_version, "Too old version: %s" % str(clientHello.client_version)
            ):
                yield result

        # there MUST be at least one value in both of those
        if not clientHello.cipher_suites or not clientHello.compression_methods:
            for result in self._sendError(AlertDescription.decode_error, "Malformed Client Hello message"):
                yield result

        # client hello MUST advertise uncompressed method
        if 0 not in clientHello.compression_methods:
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Client Hello missing uncompressed method"
            ):
                yield result
        ...
        if clientHello.client_version >= (3, 3) and ext and not ext.sigalgs:
            for result in self._sendError(AlertDescription.decode_error, "Malformed signature_algorithms extension"):
                yield result
        ...
        if alpnExt:
            if not alpnExt.protocol_names:
                for result in self._sendError(AlertDescription.decode_error, "Client sent empty list of ALPN names"):
                    yield result
            for protocolName in alpnExt.protocol_names:
                if not protocolName:
                    for result in self._sendError(
                        AlertDescription.decode_error, "Client sent empty name in ALPN extension"
                    ):
                        yield result
        ...
        if sniExt and (not sniExt.extData or not sniExt.serverNames):
            for result in self._sendError(AlertDescription.decode_error, "Recevived SNI extension is malformed"):
                yield result
        if sniExt and sniExt.hostNames:
            # RFC 6066 limitation
            if len(sniExt.hostNames) > 1:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Client sent multiple host names in SNI extension"
                ):
                    yield result
            if not sniExt.hostNames[0]:
                for result in self._sendError(AlertDescription.decode_error, "Received SNI extension is malformed"):
                    yield result
            try:
                name = sniExt.hostNames[0].decode("ascii", "strict")
            except UnicodeDecodeError:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Host name in SNI is not valid ASCII"
                ):
                    yield result
            if not is_valid_hostname(name):
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Host name in SNI is not valid DNS name"
                ):
                    yield result
        ...
        if emsExt and emsExt.extData:
            for result in self._sendError(
                AlertDescription.decode_error, "Non empty payload of the Extended " "Master Secret extension"
            ):
                yield result
        ...
        if ver_ext and (3, 4) in ver_ext.versions:
            ...
            psk_modes = clientHello.getExtension(ExtensionType.psk_key_exchange_modes)
            key_share = clientHello.getExtension(ExtensionType.key_share)
            sup_groups = clientHello.getExtension(ExtensionType.supported_groups)

            pha = clientHello.getExtension(ExtensionType.post_handshake_auth)
            if pha:
                if pha.extData:
                    for result in self._sendError(
                        AlertDescription.decode_error, "Invalid encoding of post_handshake_auth extension"
                    ):
                        yield result
            ...
            if psk_modes:
                if not psk_modes.modes:
                    for result in self._sendError(
                        AlertDescription.decode_error, "Empty psk_key_exchange_modes extension"
                    ):
                        yield result
            # psk_ke
            if psk:
                if not psk.identities:
                    for result in self._sendError(AlertDescription.decode_error, "No identities in PSK extension"):
                        yield result
                if not psk.binders:
                    for result in self._sendError(AlertDescription.decode_error, "No binders in PSK extension"):
                        yield result
                if len(psk.identities) != len(psk.binders):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Number of identities does not match number of " "binders in PSK extension",
                    ):
                        yield result
                if any(not i.identity for i in psk.identities):
                    for result in self._sendError(AlertDescription.decoder_error, "Empty identity in PSK extension"):
                        yield result
                if any(not i for i in psk.binders):
                    for result in self._sendError(AlertDescription.decoder_error, "Empty binder in PSK extension"):
                        yield result
                if psk is not clientHello.extensions[-1]:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "PSK extension not last in client hello"
                    ):
                        yield result
                if not psk_modes:
                    for result in self._sendError(
                        AlertDescription.missing_extension, "PSK extension without psk_key_exchange_modes " "extension"
                    ):
                        yield result
            ...
            if not key_exchange:
                if not sup_groups:
                    for result in self._sendError(
                        AlertDescription.missing_extension, "Missing supported_groups extension"
                    ):
                        yield result
                if not key_share:
                    for result in self._sendError(AlertDescription.missing_extension, "Missing key_share extension"):
                        yield result

                if not sup_groups.groups:
                    for result in self._sendError(AlertDescription.decode_error, "Empty supported_groups extension"):
                        yield result
                if key_share.client_shares is None:
                    for result in self._sendError(AlertDescription.decode_error, "Empty key_share extension"):
                        yield result
                ...
                if TLS_1_3_FORBIDDEN_GROUPS.intersection(sup_groups.groups):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Client advertised in TLS 1.3 Client Hello a key " "exchange group forbidden in TLS 1.3",
                    ):
                        yield result

                # Check key_share
                mismatch = next((i for i in key_share.client_shares if i.group not in sup_groups.groups), None)
                if mismatch:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Client sent key share for "
                        "group it did not advertise "
                        "support for: {0}".format(GroupName.toStr(mismatch)),
                    ):
                        yield result
                ...
                if len(set(key_share_ids)) != len(key_share_ids):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "Client sent multiple key shares for the same " "group"
                    ):
                        yield result
                ...
                if key_share_ids != [i for i in group_ids if i not in diff]:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Client sent key shares in different order than " "the advertised groups.",
                    ):
                        yield result

                sig_algs = clientHello.getExtension(ExtensionType.signature_algorithms)
            ...
            if not key_exchange:
                for result in self._sendError(AlertDescription.missing_extension, "Missing extension"):
                    yield result

            early_data = clientHello.getExtension(ExtensionType.early_data)
            if early_data:
                if early_data.extData:
                    for result in self._sendError(AlertDescription.decode_error, "malformed early_data extension"):
                        yield result
                if not psk:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "early_data without PSK extension"
                    ):
                        yield result
        ...
        high_ver = None
        if ver_ext:
            high_ver = getFirstMatching(settings.versions, ver_ext.versions)
            if not high_ver:
                for result in self._sendError(
                    AlertDescription.protocol_version, "supported_versions did not include version we " "support"
                ):
                    yield result
        if high_ver:
        ...
        if version < settings.maxVersion and CipherSuite.TLS_FALLBACK_SCSV in clientHello.cipher_suites:
            for result in self._sendError(AlertDescription.inappropriate_fallback):
                yield result
        ...
        if sni_ext:
            ...
            if sni and sni != name:
                alert = Alert().create(AlertDescription.unrecognized_name, AlertLevel.warning)
        ...
        if clientGroups is not None:
            ...
            if not clientGroups:
                for result in self._sendError(
                    AlertDescription.decode_error, "Received malformed supported_groups extension"
                ):
                    yield result
            ...
            if not ffGroupIntersect:
                if clientGroups and any(i for i in clientGroups if i in range(256, 512)):
        ...
        if heartbeat_ext:
            if heartbeat_ext.mode == HeartbeatMode.PEER_ALLOWED_TO_SEND:
                ...
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Received invalid value in Heartbeat extension"
                ):
                    yield result
            self.heartbeat_supported = True
            self.heartbeat_can_receive = True

        size_limit_ext = clientHello.getExtension(ExtensionType.record_size_limit)
        if size_limit_ext:
            if size_limit_ext.record_size_limit is None:
                for result in self._sendError(AlertDescription.decode_error, "Malformed record_size_limit extension"):
                    yield result
            if not 64 <= size_limit_ext.record_size_limit:
                for result in self._sendError(
                    AlertDescription.illegal_parameter, "Invalid value in record_size_limit extension"
                ):
                    yield result
            if settings.record_size_limit:
                ...
                if version >= (3, 4):
                    ...
                    self._send_record_limit = min(2**14, size_limit_ext.record_size_limit - 1)
                    ...
                    self._peer_record_size_limit = min(2**14, size_limit_ext.record_size_limit)
        ...
        if verifierDB:
            if cert_chain:
                cipherSuites += CipherSuite.getSrpCertSuites(settings, version)
            cipherSuites += CipherSuite.getSrpSuites(settings, version)
        elif cert_chain:
            if ecGroupIntersect or ffGroupIntersect:
                cipherSuites += CipherSuite.getTLS13Suites(settings, version)
            if ecGroupIntersect:
                cipherSuites += CipherSuite.getEcdsaSuites(settings, version)
                cipherSuites += CipherSuite.getEcdheCertSuites(settings, version)
            if ffGroupIntersect:
                cipherSuites += CipherSuite.getDheCertSuites(settings, version)
                cipherSuites += CipherSuite.getDheDsaSuites(settings, version)
            cipherSuites += CipherSuite.getCertSuites(settings, version)
        elif anon:
            cipherSuites += CipherSuite.getAnonSuites(settings, version)
            cipherSuites += CipherSuite.getEcdhAnonSuites(settings, version)
        elif settings.pskConfigs:
            cipherSuites += CipherSuite.getTLS13Suites(settings, version)
        else:
            assert False
        cipherSuites = CipherSuite.filterForVersion(cipherSuites, minVersion=version, maxVersion=version)
        ...
        if clientHello.session_id and sessionCache:
            ...
            try:
                ...
                if session.cipherSuite not in clientHello.cipher_suites:
                    for result in self._sendError(AlertDescription.illegal_parameter):
                        yield result
                if clientHello.srp_username:
                    if not session.srpUsername or clientHello.srp_username != bytearray(session.srpUsername, "utf-8"):
                        for result in self._sendError(AlertDescription.handshake_failure):
                            yield result
                if clientHello.server_name:
                    if not session.serverName or clientHello.server_name != bytearray(session.serverName, "utf-8"):
                        for result in self._sendError(AlertDescription.handshake_failure):
                            yield result
                if session.encryptThenMAC and not clientHello.getExtension(ExtensionType.encrypt_then_mac):
                    for result in self._sendError(AlertDescription.illegal_parameter):
                        yield result
                # if old session used EMS, new connection MUST use EMS
                if session.extendedMasterSecret and not clientHello.getExtension(ExtensionType.extended_master_secret):
                ...
                elif not session.extendedMasterSecret and clientHello.getExtension(
            ...
            if session:
                ...
                if session.encryptThenMAC:
                    self._recordLayer.encryptThenMAC = True
                    mte = TLSExtension().create(ExtensionType.encrypt_then_mac, bytearray(0))
                    extensions.append(mte)
                if session.extendedMasterSecret:
                    ems = TLSExtension().create(ExtensionType.extended_master_secret, bytearray(0))
                    ...
                    if alpnExt:
                        for protocolName in alpnExt.protocol_names:
                            ...
                            for result in self._sendError(
                                ...
                                "No commonly supported application layer" "protocol supported",
                            ):
                                yield result
                ...
                if heartbeat_ext:
                    if heartbeat_ext.mode == HeartbeatMode.PEER_ALLOWED_TO_SEND:
                        ...
                        for result in self._sendError(
                            AlertDescription.illegal_parameter, "Client sent invalid Heartbeat extension"
                        ):
                            yield result
                    heartbeat = HeartbeatExtension().create(HeartbeatMode.PEER_ALLOWED_TO_SEND)
                    self.heartbeat_can_receive = True
                    self.heartbeat_supported = True
                    extensions.append(heartbeat)
                record_limit = clientHello.getExtension(ExtensionType.record_size_limit)
                ...
                # Exchange ChangeCipherSpec and Finished messages
                for result in self._sendFinished(session.masterSecret, session.cipherSuite, settings=settings):
                    yield result
                for result in self._getFinished(session.masterSecret, session.cipherSuite):
                    yield result
        ...
        try:
            cipherSuite, sig_scheme, cert_chain, private_key = self._server_select_certificate(
                settings, clientHello, cipherSuites, cert_chain, private_key, version
            )
        except TLSHandshakeFailure as err:
            for result in self._sendError(AlertDescription.handshake_failure, str(err)):
                yield result
        except TLSInsufficientSecurity as err:
            for result in self._sendError(AlertDescription.insufficient_security, str(err)):
                yield result
        except TLSIllegalParameterException as err:
            for result in self._sendError(AlertDescription.illegal_parameter, str(err)):
                yield result

        # If an RSA suite is chosen, check for certificate type intersection
        if (
            cipherSuite in CipherSuite.certAllSuites or cipherSuite in CipherSuite.ecdheEcdsaSuites
        ) and CertificateType.x509 not in clientHello.certificate_types:
            for result in self._sendError(
                AlertDescription.handshake_failure, "the client doesn't support my certificate type"
            ):
                yield result
        ...
        if version > (3, 3):
            ...
            if share:
                share_ids = [i.group for i in share.client_shares]
                acceptable_ids = [
                    getattr(GroupName, i) for i in chain(settings.keyShares, settings.eccCurves, settings.dhGroups)
                ]
                for selected_group in acceptable_ids:
                    ...
                    supported = clientHello.getExtension(ExtensionType.supported_groups)
                    supported_ids = supported.groups
                    selected_group = next((i for i in acceptable_ids if i in supported_ids), None)
                    if not selected_group:
                        for result in self._sendError(
                            AlertDescription.handshake_failure, "No acceptable group " "advertised by client"
                        ):
                            yield result
            ...
            if hrr_ext:
                ...
                cookie = cookie.create(bytearray(b"\x00\x20") + getRandomBytes(32))
            ...
            if hrr_ext:
                ...
                hrr.create((3, 3), TLS_1_3_HRR, clientHello.session_id, cipherSuite, extensions=hrr_ext)
                ...
                if not ext:
                    for result in self._sendError(
                        AlertDescription.missing_extension, "Key share missing in " "Client Hello"
                    ):
                        yield result
                ...
                if len(ext.client_shares) != 1:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "Multiple key shares in " "second Client Hello"
                    ):
                        yield result
                if ext.client_shares[0].group != selected_group:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "Client key share does not " "match Hello Retry Request"
                    ):
                        yield result
                ...
                if cookie:
                    ...
                    for i, ext in enumerate(clientHello.extensions):
                        ...
                        for result in self._sendError(
                            ...
                            "Second client hello " "does not contain " "cookie extension",
                        ):
                            yield result

                # also padding extension may change
                old_ext = clientHello1.getExtension(ExtensionType.client_hello_padding)
                new_ext = clientHello.getExtension(ExtensionType.client_hello_padding)
                if old_ext != new_ext:
                    if old_ext is None and new_ext:
                        for i, ext in enumerate(clientHello.extensions):
                            if ext.extType == ExtensionType.client_hello_padding:
                                clientHello1.extensions.insert(i, ext)
                                break
                    elif old_ext and new_ext is None:
                        # extension was removed, so remove it here too
                        clientHello1.extensions[:] = (
                            i for i in clientHello1.extensions if i.extType != ExtensionType.client_hello_padding
                        )
                    else:
                ...
                # and binders
                old_ext = clientHello1.getExtension(ExtensionType.pre_shared_key)
                new_ext = clientHello.getExtension(ExtensionType.pre_shared_key)
                ...
                if clientHello1 != clientHello:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Old Client Hello does not " "match the updated Client " "Hello",
                    ):
                        yield result
    ...
    def _serverSRPKeyExchange(
        self, clientHello, serverHello, verifierDB, cipherSuite, privateKey, serverCertChain, settings
    ):
        """Perform the server side of SRP key exchange"""
        try:
            sigHash, serverCertChain, privateKey = self._pickServerKeyExchangeSig(
                settings, clientHello, serverCertChain, privateKey
            )
        except TLSHandshakeFailure as alert:
            for result in self._sendError(AlertDescription.handshake_failure, str(alert)):
                yield result

        keyExchange = SRPKeyExchange(cipherSuite, clientHello, serverHello, privateKey, verifierDB)

        # Create ServerKeyExchange, signing it if necessary
        try:
            serverKeyExchange = keyExchange.makeServerKeyExchange(sigHash)
        except TLSUnknownPSKIdentity:
            for result in self._sendError(AlertDescription.unknown_psk_identity):
                yield result
        except TLSInsufficientSecurity:
            for result in self._sendError(AlertDescription.insufficient_security):
                yield result
        ...
        for result in self._getMsg(ContentType.handshake, HandshakeType.client_key_exchange, cipherSuite):
            if result in (0, 1):
                yield result
            else:
                break
        try:
            ...
            for result in self._sendError(AlertDescription.illegal_parameter, "Suspicious A value"):
                yield result
    ...
    def _server_select_certificate(self, settings, client_hello, cipher_suites, cert_chain, private_key, version):
        """
        ...
        # Get client groups
        client_groups = client_hello.getExtension(ExtensionType.supported_groups)
        ...
        client_sigalgs = client_hello.getExtension(ExtensionType.signature_algorithms_cert)
        if client_sigalgs is not None:
            client_sigalgs = client_hello.getExtension(ExtensionType.signature_algorithms_cert).sigalgs
        else:
            client_sigalgs = client_hello.getExtension(ExtensionType.signature_algorithms)
            if client_sigalgs is not None:
                client_sigalgs = client_hello.getExtension(ExtensionType.signature_algorithms).sigalgs
            else:
                client_sigalgs = []

        # Get all the certificates we can offer
        alt_certs = ((X509CertChain(i.certificates), i.key) for vh in settings.virtual_hosts for i in vh.keys)
        certs = [(cert, key) for cert, key in chain([(cert_chain, private_key)], alt_certs)]
        ...
        for cert, key in certs:
            ...
            try:
                ...
                for cipher in ciphers:
                    ...
                    if (
                        client_groups
                        and any(i in range(256, 512) for i in client_groups)
                        and any(i in CipherSuite.dhAllSuites for i in client_hello.cipher_suites)
                    ):
                        raise TLSInsufficientSecurity("FFDHE groups not acceptable and no other common " "ciphers")
                    raise TLSHandshakeFailure("No mutual ciphersuite")

                # Find a signature algorithm based on the certificate
                try:
                    sig_scheme, _, _ = self._pickServerKeyExchangeSig(settings, client_hello, cert, key, version, False)
                ...
                if cert and cert.x509List[0].certAlg == "ecdsa" and client_groups and client_sigalgs:
                    ...
                    if version <= (3, 3) and curve not in client_groups:
                        raise TLSHandshakeFailure(
                            ...
                            "supported by the client: {0}".format(GroupName.toRepr(curve))
                        )

                    if version >= (3, 4):
                        if GroupName.toRepr(curve) not in ("secp256r1", "secp384r1", "secp521r1"):
                            raise TLSIllegalParameterException("Curve in public key is not supported " "in TLS1.3")
    ...
    def _serverCertKeyExchange(
        self, clientHello, serverHello, sigHashAlg, serverCertChain, keyExchange, reqCert, reqCAs, cipherSuite, settings
    ):
        # Send ServerHello, Certificate[, ServerKeyExchange]
        ...
        try:
            ...
            for result in self._sendError(AlertDescription.internal_error, str(alert)):
                yield result
        ...
        if reqCert:
            ...
            certificateRequest.create(
                [ClientCertificateType.rsa_sign, ClientCertificateType.ecdsa_sign], reqCAs, valid_sig_algs
            )
        ...
        if reqCert:
            if self.version == (3, 0):
                for result in self._getMsg(
                    (ContentType.handshake, ContentType.alert), HandshakeType.certificate, CertificateType.x509
                ):
                    if result in (0, 1):
                        yield result
                    else:
                ...
                if isinstance(msg, Alert):
                    ...
                    alert = msg
                    if alert.description != AlertDescription.no_certificate:
                        self._shutdown(False)
                        raise TLSRemoteAlert(alert)
                elif isinstance(msg, Certificate):
                    clientCertificate = msg
                    if clientCertificate.cert_chain and clientCertificate.cert_chain.getNumCerts() != 0:
        ...
        if clientCertChain:
            ...
            if self.version == (3, 3):
                valid_sig_algs = self._sigHashesToList(settings, certList=clientCertChain)
                if certificateVerify.signatureAlgorithm not in valid_sig_algs:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, "Invalid signature algorithm in Certificate " "Verify"
                    ):
                        yield result
                signatureAlgorithm = certificateVerify.signatureAlgorithm
            if not signatureAlgorithm and clientCertChain.x509List[0].certAlg == "ecdsa":
            ...
            for result in self._check_certchain_with_settings(clientCertChain, settings):
                if result in (0, 1):
                    yield result
                else:
                    break
            ...
            if signatureAlgorithm and signatureAlgorithm in (SignatureScheme.ed25519, SignatureScheme.ed448):
                hash_name = "intrinsic"
                salt_len = None
                padding = None
                ver_func = public_key.hashAndVerify
            elif not signatureAlgorithm or signatureAlgorithm[1] != SignatureAlgorithm.ecdsa:
                ...
                verify_bytes = verify_bytes[: public_key.public_key.curve.baselen]
                padding = None
                salt_len = None
                ver_func = public_key.verify

            if not ver_func(certificateVerify.signature, verify_bytes, padding, hash_name, salt_len):
                for result in self._sendError(AlertDescription.decrypt_error, "Signature failed to verify"):
                    yield result
    ...
    def _serverFinished(
        self, premasterSecret, clientRandom, serverRandom, cipherSuite, cipherImplementations, nextProtos, settings
    ):
        if self.extendedMasterSecret:
            cvhh = self._certificate_verify_handshake_hash
        ...
        # Exchange ChangeCipherSpec and Finished messages
        for result in self._getFinished(masterSecret, cipherSuite, expect_next_protocol=nextProtos is not None):
            yield result

        for result in self._sendFinished(masterSecret, cipherSuite, settings=settings):
            yield result
    ...
    def _sendFinished(self, masterSecret, cipherSuite=None, nextProto=None, settings=None):
        ...
        verifyData = calc_key(
            self.version, masterSecret, cipherSuite, label, handshake_hashes=self._handshake_hash, output_length=12
        )
    ...
    def _getFinished(self, masterSecret, cipherSuite=None, expect_next_protocol=False, nextProto=None):
        ...
        if changeCipherSpec.type != 1:
            for result in self._sendError(AlertDescription.illegal_parameter, "ChangeCipherSpec type incorrect"):
                yield result
        ...
        if expect_next_protocol:
            for result in self._getMsg(ContentType.handshake, HandshakeType.next_protocol):
                if result in (0, 1):
                    yield result
            if result is None:
                for result in self._sendError(AlertDescription.unexpected_message, "Didn't get NextProtocol message"):
                    yield result
        ...
        for result in self._getMsg(ContentType.handshake, HandshakeType.finished):
            if result in (0, 1):
                yield result
        finished = result
        if finished.verify_data != verifyData:
            for result in self._sendError(AlertDescription.decrypt_error, "Finished message is incorrect"):
                yield result
        ...
        try:
            ...
            if checker:
                try:
                    ...
                    alert = Alert().create(AlertDescription.close_notify, AlertLevel.fatal)
                    for result in self._sendMsg(alert):
                        yield result
                    raise
    ...
    @staticmethod
    def _pickServerKeyExchangeSig(
        settings, clientHello, certList=None, private_key=None, version=(3, 3), check_alt=True
    ):
        """Pick a hash that matches most closely the supported ones"""
        hashAndAlgsExt = clientHello.getExtension(ExtensionType.signature_algorithms)
        ...
        for certs, key in chain([(certList, private_key)], alt_certs):
            supported = TLSConnection._sigHashesToList(settings, certList=certs, version=version)

            for schemeID in supported:
                if schemeID in hashAndAlgsExt.sigalgs:
                    name = SignatureScheme.toRepr(schemeID)
                    if not name and schemeID[1] in (
    ...
    @staticmethod
    def _sigHashesToList(settings, privateKey=None, certList=None, version=(3, 3)):
        ...
        if not certType or certType == "ecdsa":
            for hashName in settings.ecdsaSigHashes:
                ...
                if publicKey and version > (3, 3):
                    ...
                    matching_hash = TLSConnection._curve_name_to_hash_name(curve)
                    if hashName != matching_hash:
                        continue

                sigAlgs.append((getattr(HashAlgorithm, hashName), SignatureAlgorithm.ecdsa))
        ...
        if not certType or certType == "dsa":
            for hashName in settings.dsaSigHashes:
                ...
                sigAlgs.append((getattr(HashAlgorithm, hashName), SignatureAlgorithm.dsa))
            ...
            for schemeName in settings.rsaSchemes:
                ...
                for hashName in settings.rsaSigHashes:
                    ...
                    try:
                        ...
                        if schemeName == "pss" and hashName == "sha512" and privateKey and privateKey.n < 2**2047:
                            continue
                        # advertise support for both rsaEncryption and RSA-PSS OID
                        # key type
                        if certType != "rsa-pss":
                            sigAlgs.append(getattr(SignatureScheme, "rsa_{0}_rsae_{1}".format(schemeName, hashName)))
                        if certType != "rsa":
                            sigAlgs.append(getattr(SignatureScheme, "rsa_{0}_pss_{1}".format(schemeName, hashName)))
                    except AttributeError:
                        if schemeName == "pkcs1":
                            sigAlgs.append((getattr(HashAlgorithm, hashName), SignatureAlgorithm.rsa))
                        continue
        return sigAlgs
    ...
    def _curve_name_to_hash_name(curve_name):
        ...
        if curve_name == "NIST521p":
            return "sha512"
        raise TLSIllegalParameterException("Curve {0} is not supported in TLS 1.3".format(curve_name))
